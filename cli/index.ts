...
function bootstrap() {
  ...
  cli
    .command('[template] [projectName]', 'Create a new project')
  ...
  cli
    .command('new [template] [projectName]', 'Create a new project')
    .option('-t, --template <template>', 'Specify template name')
    .action((template: string | undefined, projectName: string | undefined, options: any) => {
      if (template && !options.template) {
        options.template = template
  ...
  cli.command('list', 'List all available templates').action(async () => {
    try {
